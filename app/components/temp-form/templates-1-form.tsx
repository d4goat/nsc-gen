...
import Templates1 from "../templates/templates-1";
...
const formSchema = z.object({
    ...
    coreCompetency: z.array(z.string())
});
...
// Helper component defined outside the main component to prevent rerendering loops and state/focus loss
...
const WorkExperienceItem = ({ control, register, index, remove, errors }: WorkExperienceItemProps) => {
    const { fields: listFields, append: appendListItem, remove: removeListItem } = useFieldArray({
        control,
        name: `workExperience.${index}.workList` as never
    });

    return (
        <div className="grid gap-4 border p-4 rounded-md relative">
            <Button type="button" variant="ghost" size="icon" className="absolute top-2 right-2" onClick={remove}>
                <Trash className="h-4 w-4 text-red-500" />
            </Button>
            <FormInput {...register(`workExperience.${index}.workTitle`)} label="Work Title" placeholder="Work Title Name" error={errors.workExperience?.[index]?.workTitle?.message} />
            ...
            <div className="space-y-2">
                <div className="flex justify-between items-center">
                    <label className="text-sm font-medium">Work Description List</label>
                    <Button type="button" variant="outline" size="sm" onClick={() => appendListItem("")}>
                        <Plus className="h-3 w-3 mr-1" /> Add Item
                    </Button>
                </div>
                <div className="space-y-2">
                    {listFields.map((subField, subIndex) => (
                        <div key={subField.id} className="flex gap-2 items-end">
                            <FormInput {...register(`workExperience.${index}.workList.${subIndex}` as any)} label={`Work List ${subIndex + 1}`} placeholder="Description item" />
                            <Button type="button" variant="ghost" size="icon" onClick={() => removeListItem(subIndex)}>
                                <Trash className="h-4 w-4 text-red-500" />
                            </Button>
                        </div>
                    ))}
...
export function Templates1Form() {
    ...
    // Special handling for array of strings (coreCompetency)
    ...
    // For string arrays, we can use useFieldArray if we name it "coreCompetency" and append "string".
    const { fields: competencyFields, append: appendCompetency, remove: removeCompetency } = useFieldArray({
        control,
        name: "coreCompetency" as never // Cast to avoid TS issues if strictly typed to string[]
    });
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 max-w-7xl mx-auto p-4 print:p-0 print:block">
            {/* Form Column */}
            <form onSubmit={handleSubmit(onSubmit)} className="lg:col-span-7 space-y-6 print:hidden">
                <Card className={`${SaweriaClass}`}>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="grid gap-4 border p-4 rounded-md">
                            ...
                            <FormInput {...register("title.title")} label="Name" placeholder="Name" error={errors.title?.title?.message} />
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <FormInput {...register("title.contact.phone")} label="Phone" placeholder="Phone" error={errors.title?.contact?.phone?.message} />
                                <FormInput {...register("title.contact.mail")} label="Email" placeholder="Email" error={errors.title?.contact?.mail?.message} />
                                <FormInput {...register("title.contact.street")} label="Street" placeholder="Street" error={errors.title?.contact?.street?.message} />
                            </div>
                        </div>

                        {/* Profile */}
                        <FormInput {...register("profile")} type="textarea" rows={6} label="Professional Summary" placeholder="Professional Summary" error={errors.profile?.message} />
                        ...
                        <div className="space-y-4">
                            ...
                            {studyFields.map((field, index) => (
                                <div key={field.id} className="grid gap-4 border p-4 rounded-md relative">
                                    ...
                                    <FormInput {...register(`studyExperience.${index}.title`)} label="Degree / School" placeholder="Degree / School Name" error={errors.studyExperience?.[index]?.title?.message} />
                                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-center">
                                ...
                                <Button type="button" variant="outline" size="sm" onClick={() => appendCert("")}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                {certFields.map((field, index) => (
                                    <div key={field.id} className="flex gap-2 items-end">
                                        <FormInput {...register(`certificate.${index}` as any)} label={`Certificate ${index + 1}`} placeholder="Certificate Name" />
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-center">
                                ...
                                <Button type="button" variant="outline" size="sm" onClick={() => appendCompetency("")}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                {competencyFields.map((field, index) => (
                                    <div key={field.id} className="flex gap-2 items-end">
                                        <FormInput {...register(`coreCompetency.${index}` as any)} label={`Core Competency ${index + 1}`} placeholder="Skill" />
            ...
            <div className="lg:col-span-5 print:block print:w-full">
                <div className="sticky top-24 print:static">
                    <h3 className="text-lg font-bold mb-4 print:hidden text-center lg:text-left">Live Preview</h3>
                    <Templates1
                        title={watchedValues.title || { title: "", contact: { street: "", phone: "", mail: "" } }}
                        profile={watchedValues.profile || ""}
                        workExperience={watchedValues.workExperience || []}
                        studyExperience={watchedValues.studyExperience || []}
                        certificate={watchedValues.certificate || []}
                        coreCompetency={watchedValues.coreCompetency || []}
                    />
                </div>
            </div>
        </div>
